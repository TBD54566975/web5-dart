...
class Question {
  ...
  factory Question.decode(Uint8List buf, int offset) {
    final result = QuestionCodec.decode(buf, offset: offset);
    return result.value;
  }

  Uint8List encode({Uint8List? buf, int offset = 0}) {
    final result = QuestionCodec.encode(this, input: buf, offset: offset);
    return result.value;
  }
...
class QuestionCodec {
  static DecodeResult<Question> decode(Uint8List buf, {int offset = 0}) {
  ...
  static EncodeResult encode(
    ...
    final n = RecordNameCodec.encode(question.name, input: buf, offset: offset);
    offset += n.offset;
